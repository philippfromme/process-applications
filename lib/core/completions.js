...
export default class Completions {
  ...
  get(ref) {
    ...
    const items = this._indexer.getItems();

    if (type === 'bpmn') {
      return items.filter(item => {
        return item.parsedValue.type === 'bpmn' && item.parsedValue.processIds.find(id => id.startsWith(value));
      }).map(item => {
        return item.parsedValue.processIds.find(id => id.startsWith(value));
      });
    } else if (type === 'dmn') {
      return items.filter(item => {
        return item.parsedValue.type === 'dmn' && item.parsedValue.decisionIds.find(id => id.startsWith(value));
      }).map(item => {
        return item.parsedValue.decisionIds.find(id => id.startsWith(value));
      });
    } else if (type === 'form') {
      return items.filter(item => {
        return item.parsedValue.type === 'form' && item.parsedValue.formId.startsWith(value);
      }).map(item => {
        return item.parsedValue.formId;
      });
    }

    this._logger.log('cannot complete value', value);
...
function findNode(root, position) {
  ...
  visit(
    root,
    n => (
      (n.children || []).every(ch => !inRange(position, ch.position))
    ) && inRange(position, n.position),
    n => node = n
  );
